...
async function startServer() {
  ...
  function startAutoRecordingForCamera(cam: Camera) {
    ...
    console.log(`[Auto Recorder 24/7] Gravando bloco automático real de ${autoRecordingDurationSec}s para '${cam.name}' (${cam.id})...`);
    const proc = spawn('ffmpeg', ffmpegArgs);
  ...
  setTimeout(checkAndStartAllAutoRecordings, 1500);
