...
function startCameraRtspStream(cam: Camera, forceRestart = false) {
  ...
  const recDir = path.join(process.cwd(), 'public', 'recordings');
  if (!fs.existsSync(recDir)) {
    try { fs.mkdirSync(recDir, { recursive: true }); } catch (e) {}
  }
  const segmentPattern = path.join(recDir, `rec_auto_${cleanCamId}_%s.mp4`);

  ffmpegArgs.push(
    '-analyzeduration', '2000000',
    '-probesize', '2000000',
    '-i', streamSource,
    // Output 1: Live HLS Stream for Web Player
    '-map', '0:v:0?',
    '-c:v', 'copy',
    '-f', 'hls',
    '-hls_time', '2',
    ...
    hlsPath,
    // Output 2: Continuous 24/7 Rolling Cloud MP4 Recording Slices (300 seconds = 5 minutes)
    '-map', '0:v:0?',
    '-c:v', 'copy',
    '-f', 'segment',
    '-segment_time', '300',
  ...
  const handleProcessEnded = (code: number | null, reason: string) => {
    console.log(`[FFmpeg ITL] Processo HLS da câmera '${key}' finalizou (${reason}, código ${code})`);
    logList.push(`Processo HLS finalizado (${reason})`);
    activeFfmpegProcesses.delete(key);
    activeRtspUrls.delete(key);

    // Auto-reconnect stream worker after 4s
    setTimeout(() => {
      if (cam) {
        console.log(`[FFmpeg ITL Auto-Reconnect] Reconectando fluxo HLS para '${cam.name}' (${key})...`);
        startCameraRtspStream(cam);
      }
    }, 4000);
  };

  proc.on('exit', (code) => handleProcessEnded(code, 'exit'));
  proc.on('close', (code) => handleProcessEnded(code, 'close'));
  proc.on('error', (err) => {
...
async function startServer() {
  ...
  // Auto-scan existing disk recordings on startup and periodically to ensure all saved MP4s appear in cloud vault
  function scanAndRegisterDiskRecordings() {
    try {
      ...
      for (const file of files) {
        ...
        const existing = recordings.find((r) => r.streamUrl === relativeUrl);
        if (existing) continue;

        const filePath = path.join(recordingsDir, file);
        const stats = fs.statSync(filePath);
        if (stats.size < 2000) continue; // Skip corrupted empty files

        // Find associated camera
        const matchedCam = cameras.find((c) => {
          const cleanId = c.id.replace(/[^a-zA-Z0-9_-]/g, '_');
          return file.includes(cleanId) || file.includes(c.id);
        }) || cameras[0];

        if (!matchedCam) continue;

        const timestampStr = file.replace(/[^0-9]/g, '');
        let startTimeDate = stats.mtime;
        if (timestampStr && timestampStr.length >= 10) {
          const parsedTs = parseInt(timestampStr.slice(-13), 10);
          if (!isNaN(parsedTs) && parsedTs > 1500000000000) {
            startTimeDate = new Date(parsedTs);
          }
        }

        const fileSizeMB = +(stats.size / (1024 * 1024)).toFixed(1);
        const thumbFileName = file.replace('.mp4', '.jpg').replace('rec_', 'thumb_');
        const thumbPath = path.join(recordingsDir, thumbFileName);

        if (!fs.existsSync(thumbPath)) {
          try {
            execSync(`ffmpeg -y -ss 00:00:01 -i "${filePath}" -vframes 1 -q:v 2 "${thumbPath}"`, { stdio: 'ignore' });
          } catch (e) {}
        }

        const thumbUrl = fs.existsSync(thumbPath)
          ? `/recordings/${thumbFileName}`
          : (matchedCam.thumbnailUrl || 'https://images.unsplash.com/photo-1557597774-9d273605dfa9?w=800');

        const endTimeDate = new Date(startTimeDate.getTime() + Math.min(300, Math.round(fileSizeMB * 30)) * 1000);

        const newRec: CloudRecording = {
          id: `rec-disk-${matchedCam.id}-${Date.now()}-${Math.floor(Math.random()*1000)}`,
          cameraId: matchedCam.id,
          cameraName: matchedCam.name,
          startTime: formatDateTime(startTimeDate),
          endTime: formatDateTime(endTimeDate),
          durationSeconds: Math.min(300, Math.round(fileSizeMB * 30)),
          fileSizeMB,
          thumbnailUrl: thumbUrl,
          streamUrl: relativeUrl,
          isE2EELocked: matchedCam.isE2EEEncrypted ?? true,
          tags: ['Gravação Automática 24/7', 'Nuvem Real HD', matchedCam.location || 'Central ITL'],
        };
      ...
      if (added > 0) {
        ...
        saveToLocalFile();
        console.log(`[Auto Recorder Disk Scanner] Registrados ${added} blocos de gravação existentes no disco.`);
      }
    } catch (err) {
      console.error('[Auto Recorder Disk Scanner Warning]', err);
    }
  }

  // Scan disk recordings immediately and every 10s
  scanAndRegisterDiskRecordings();
  setInterval(scanAndRegisterDiskRecordings, 10000);

  function checkAndStartAllAutoRecordings() {
    cameras.forEach((cam) => {
      if (cam && cam.cloudRecordingsActive !== false) {
        // Guarantee unified FFmpeg worker (Live HLS + 24/7 MP4 cloud slices) is active
        startCameraRtspStream(cam);
      }
    });
  }

  // Start continuous 24/7 background stream and recording worker for all cameras immediately and every 5s
